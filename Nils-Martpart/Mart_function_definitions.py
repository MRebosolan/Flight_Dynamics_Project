...
def redundant_shear_flow(q_b_s1, s1, q_b_s4, s4, aileron_radius, aileron_angle_radians, aileron_height, area_circular_section, area_triangular_section, ratio_between_redundant_shear_flows):
    ...
    for i in range(len(s1)):
        moment_sum1 = moment_sum1+s1[1]*q_b_s1[i]*aileron_radius*2
    for i in range(len(s4)):
        moment_sum2 = moment_sum2+s4[i]*q_b_s4[i]*np.sin(aileron_angle_radians)*aileron_height/2
    redundant_shear_flow_circular_section = -moment_sum1/(2*area_circular_section+2*area_triangular_section/ratio_between_redundant_shear_flows)
